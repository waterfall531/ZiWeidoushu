...
Horoscope.prototype.assignPeriod = function() {
    ...
    if (((this.bornHeavenlyStem) % 2 === 1 && this.gender) || ((this.bornHeavenlyStem) % 2 === 0 && !this.gender)) {
        // 陽男陰女順行
        // 本大限命宮宮支
        ...
        // 陰男陽女逆行
        // 本大限命宮宮支
        this.period_palace_id = (this.main_palace_idx - this.period_no + 1 + 12) % 12;
        for (var i = 0; i < this.palace.length; i++) {
            // 目前工作中的宮位
...
Horoscope.prototype.assignStars = function() {
    // 起紫微星
    var emperorLocation = HoroscopeUtil.getEmperorLocation(this.five_elem_no, this.lunarBirth);
    // 起天府星
    var ministerLocation = (emperorLocation > 6) ? (18 - emperorLocation) : (6 - emperorLocation);
    // 紫微系逆行
    //console.info(this.palace[0].primaryStar);
    this.palace[(emperorLocation + 12) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[0]);
    this.palace[(emperorLocation + 12 - 1) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[1]);
    this.palace[(emperorLocation + 12 - 3) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[2]);
    this.palace[(emperorLocation + 12 - 4) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[3]);
    this.palace[(emperorLocation + 12 - 5) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[4]);
    this.palace[(emperorLocation + 12 - 8) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[5]);
    // 天府系順行
    this.palace[(ministerLocation) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[6]);
    this.palace[(ministerLocation + 1) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[7]);
    this.palace[(ministerLocation + 2) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[8]);
    this.palace[(ministerLocation + 3) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[9]);
    this.palace[(ministerLocation + 4) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[10]);
    this.palace[(ministerLocation + 5) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[11]);
    this.palace[(ministerLocation + 6) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[12]);
    this.palace[(ministerLocation + 10) % 12].addPrimaryStar(Dictionary.PRIMARY_STAR[13]);
    //
    // 其他星
    // 祿存
    ...
    // 博士十二星
    // 陽男陰女順行
    if ((this.bornEarthlyBranch % 2 === 1 && this.gender) || (this.bornEarthlyBranch % 2 === 0 && !this.gender)) {
        for (var i = 0; i < 12; i++) {
            this.palace[(cofferLocation + i) % 12].addWizard(Dictionary.TWELVE_WIZARD[i]);
        }
    } else { // 陰男陽女逆行
        for (var i = 0; i < 12; i++) {
            this.palace[(cofferLocation + 12 - i) % 12].addWizard(Dictionary.TWELVE_WIZARD[i]);
        }
    }
    //
    var loc = new Array();
    // 華蓋
    loc = HoroscopeUtil.getOtherStarLocation(this.main_palace_idx, this.body_palace_idx, this.bornHeavenlyStem, this.bornEarthlyBranch, this.bornMonth + 2, this.lunarBirth[2], this.bornHour);
    ...
    // 火星/鈴星
    loc = HoroscopeUtil.getFlameStarLocation(this.bornEarthlyBranch, this.bornHour);
    this.palace[loc[0]].addBadStar(Dictionary.BAD_STAR[2]);
    this.palace[loc[1]].addBadStar(Dictionary.BAD_STAR[3]);
    //
    // 地空/地劫
    loc = HoroscopeUtil.getHollowStarLocation(this.bornHeavenlyStem, this.bornHour);
    ...
    loc = HoroscopeUtil.getSecondaryStarLocation(this.main_palace_idx, this.body_palace_idx, this.bornHeavenlyStem, this.bornEarthlyBranch, this.bornMonth + 2, this.lunarBirth[2], this.bornHour);
...
// 建立命盤
// 取得排盤農曆日期
Horoscope.prototype.create = function(_name, _gender, _western, _birth, _bornHour, _isLeapMonth) {
    var _horoscope = this;
    this.name = _name;
    this.gender = _gender;
    //
    ...
    cal.convertDate(0, currentDate.getFullYear(), currentDate.getMonth() + 1, currentDate.getDate(), false);
    //
...
// 國農曆日期轉換
Horoscope.prototype.init = function(_western, _birth, _bornHour, _isLeapMonth) {
    var _horoscope = this;
    ...
    if (_western) { // 西曆
        ...
        var intervalID = setInterval(function() {
            if (!cal.isRunning()) {
                clearInterval(intervalID);
                // 檢查否成功轉換
                if (cal.lunarDate.year <= 0 || cal.lunarDate.month <= 0 || cal.lunarDate.day <= 0) {
                    alert("輸入國曆日期不正確！");
                    return;
                }
                // 保留原始轉換的生辰(月份直接用)
                _horoscope.lunarBirth_org = [cal.lunarDate.year, cal.lunarDate.month, cal.lunarDate.day];
                _horoscope.lunarBirth = [cal.lunarDate.year, cal.lunarDate.month, cal.lunarDate.day];
                _horoscope.isLeapMonth = cal.isLeapMonth;
                // 開始處理閏月問題
                console.info("農曆生日: "+_horoscope.lunarBirth[0]+"年"+_horoscope.lunarBirth[1]+"月"+_horoscope.lunarBirth[2]+"日"+Dictionary.LEAP_MONTH_MODE);
                if (_horoscope.isLeapMonth) {
                    ...
                    //
                    console.info('M: '+_horoscope.bornMonth);
                } else {
                    _horoscope.bornMonth = _horoscope.lunarBirth[1];
                }
                // 開始排盤
        ...
        var intervalID = setInterval(function() {
            if (!cal.isRunning()) {
                clearInterval(intervalID);
                _horoscope.westernBirth = [cal.solarDate.getFullYear(), cal.solarDate.getMonth() + 1, cal.solarDate.getDate()];
                _horoscope.isLeapMonth = cal.isLeapMonth;
                // 開始排盤
                _horoscope.caculate();
            }
...
Horoscope.prototype.caculate = function() {
    ...
    if (this.main_palace_idx > 0 && this.main_palace_idx < 8) {
        ...
        this.internalStar = Dictionary.INNATE_MASTER[(13 - this.main_palace_idx) % 12];
    }
    // 安身主（生年地支）(13 + (year - 1900) % 12) % 12; // 生年地支
    this.externalStar = Dictionary.ACQUIRED_MASTER[((5 + this.bornEarthlyBranch) % 6) % 6];
    // 起寅首（生年天干）(17 + (year - 1900) % 10) % 10
    var firstMonthHeavenlyStemId = HoroscopeUtil.getFirstMonthHeavenlyStemId(this.bornHeavenlyStem); // 寅(月)首
    // 十二宮
    // 安干支及本命宮名
    // i=0為亥位，i=2為子位...
    for (var i = 0; i < 12; i++) {
    ...
    // 起五行局（要先安完各宮干支）
    // 以命宮的干支來決定
    this.five_elem_no = HoroscopeUtil.getFiveElementNO(this.palace[this.main_palace_idx].getHeavenklyStem(), this.palace[this.main_palace_idx].getEarthlyBranch());
    //
    // 決定為第幾大限
    ...
    // 小限男順女逆行
    if (this.gender) {
        this.annual_palace_id = ((this.virtualAge % 12) + this.firstYearId + 11) % 12;
    } else {
        this.annual_palace_id = ((this.virtualAge % 12) + this.firstYearId + 1) % 12;
    }
    // 安大限，陽男陰女順行, 陰男陽女逆行
...
Horoscope.getHoroscopeHtml = function(name, gender, westen, birthDate, bornHour, isLeapMonth, callback) {
