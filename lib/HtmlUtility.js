...
HtmlUtility.transSummaryToHTML = function(horoscope) {
    ...
    */
    ...
    content += '<div class="summary">';
    content += '國曆 ';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(westBirth[0] - 1911) + '</span>年';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(westBirth[1]) + '</span>月';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(westBirth[2]) + '</span>日 ';
    content += '<span class="summary_tb">' + Dictionary.EARTHLY_BRANCH[horoscope.getBornHour()] + '</span>時';
    content += '</div>';
    /*
    <div class="summary">
    ...
    */
    ...
    content += '<div class="summary">';
    content += '農曆 ';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(lunarBirth[0] - 1911) + '</span>年';
    if (horoscope.isLeapMonth) {
        content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(lunarBirth[1]) + "</div>";
        content += "<div class='summary_leapmonth' >閏</div>";
    } else {
        content += "<div class='summary_tb' >" + HtmlUtility.to2DigFormat(lunarBirth_Org[1]) + " </div>";
    }
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(lunarBirth[1]) + '</span>月';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(lunarBirth[2]) + '</span>日 ';
    content += '<span class="summary_tb">' + Dictionary.EARTHLY_BRANCH[horoscope.getBornHour()] + '</span>時';
    content += '</div>';
    /*
    <div class="summary">
    ...
    */
    content += '<div class="summary">';
    content += '年齡 ';
    content += '<span class="summary_tb">' + HtmlUtility.to2DigFormat(horoscope.getVirtualAge()) + '</span>歲';
    content += '</div>';
    /*
    <div class="summary">
    命主 天同
    ...
    */
    var five_elem_case = Dictionary.getFiveElementCase(horoscope.getFiveElementNo());
    content += '<div class="summary">';
    ...
    */
    content += '<div class="summary">';
    content += '歲次 ' + Dictionary.HEAVENLY_STEM[horoscope.getBornHeavenlyStem()] + Dictionary.EARTHLY_BRANCH[horoscope.getBornEarthlyBranch()];
    content += '</div>';
    var palace = horoscope.getPalace();
    /*
    <div class="summary">
    ...
    */
    content += '<div class="summary">';
    content += '大限  ' + Dictionary.HEAVENLY_STEM[palace[horoscope.period_palace_id].getHeavenklyStem()] + Dictionary.EARTHLY_BRANCH[palace[horoscope.period_palace_id].getEarthlyBranch()];
    content += '</div>';
    return content;
};
...
HtmlUtility.transPalaceToHTML = function(palace) {
    ...
    */
    ...
    for (var i = 0; i < 8; i++) {
        _internal_year_string = _internal_year_string.concat((_internal_year + 12 * i) + " ");
    }
    ...
    */
    ...
    for (var i = 0; i < primaryStar.length; i++) {
        if (primaryStar[i] !== null) {
            ...
            if (idx >= 0) { // 帶有四化
                content += "<span class='transform_star' >" + Dictionary.TRANSFORM_STAR[transformStarIdx[idx]] + "</span>";
            }
            content += '</div>';
        }
    }
