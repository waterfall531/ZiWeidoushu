...
// 某年的第n個節氣為幾日(從0小寒起算)
LunarCalendar.SOLAR_TERM_BASE = [
  4, 19, 3, 18, 4, 19, 4, 19, 4, 20, 4, 20, 6, 22, 6, 22, 6, 22, 7, 22, 6, 21,
  6, 21,
];
LunarCalendar.SOLAR_TERM_IDX =
...
// 轉換成農曆日期
// 好像會少一天？
LunarCalendar.prototype.convertToLunarDate = function (
  year,
  month,
  month_of_day
) {
...
// 轉換成國曆日期
LunarCalendar.prototype.convertToSolarDate = function (
  year,
  month,
  month_of_day,
  isLeapMonth
  ...
  if (isLeapMonth && this.getLeapMonth(year) == month) {
    // 如果是閏月，要再加上閏月的天數
...
LunarCalendar.prototype.getSolarTerm = function (year, n) {
  return (
    LunarCalendar.SOLAR_TERM_BASE[n] +
    Math.floor(
      LunarCalendar.SOLAR_TERM_OS.charAt(
        (Math.floor(LunarCalendar.SOLAR_TERM_IDX.charCodeAt(year - 1900)) -
          48) *
          24 +
          n
      )
...
// 日期轉換
// mode, 0: 國轉農, 1: 農轉國
LunarCalendar.prototype.convertDate = function (
  mode,
  year,
  month,
  month_of_day,
...
LunarCalendar.prototype.getStemBranchDate = function () {
  ...
  if (
    ...
    (this.solarDate.getMonth() == 1 && term_2 > this.solarDate.getDate())
  ) {
    // 未過立春
    year -= 1;
  } else {
  ...
  for (var i = 0; i <= this.solarDate.getMonth(); i++) {
    ...
    term_firstDay = this.getSolarTerm(
  ...
  //
  month = (month + 10) % 12;
  if (term_firstDay <= this.solarDate.getDate()) {
    // 已過節，算下一個月
    month = (month + 1) % 12;
  }
  //
